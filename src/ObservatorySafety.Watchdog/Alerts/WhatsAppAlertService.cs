  ...
  public class WhatsAppAlertService : IAlertService
  {
    ...
    private void EnsureInitialized()
    {
      ...
      if (!string.IsNullOrWhiteSpace(_sid) && !string.IsNullOrWhiteSpace(_token))
      {
        ...
        _initialized = true;
        _logger.LogInformation("WhatsAppAlertService initialized with Twilio SID: {Sid}", _sid);
      }
    }
    ...
    public async Task SendAlertAsync(string title, string message, CancellationToken cancellationToken)
    {
      ...
      {
        _logger.LogWarning("WhatsAppAlertService is not properly configured. Alert not sent.");
        return;
      }
      ...
      _logger.LogInformation("WhatsApp alert sent to {ToNumber}: {Body}", _toNumber, body);
    }
  }
}
